...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, modality, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
        ...
        if modality == 'audio':
            ...
            self.compress_layer = nn.Sequential(
                ...
                nn.Conv2d(128, 512, kernel_size=(3, 3), stride=(2, 2), padding=0),
                nn.ReLU(inplace=True),
            )

        # Visual Modality Processing
        elif modality == 'visual' or modality == 'image':
        ...
        # Ensure max pooling for both modalities
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        if self.modality == 'audio':
            x = x.unsqueeze(1)  # Ensure single channel for audio
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.relu(x)
            x = self.maxpool(x)
            x = self.layer1(x)
            x = self.layer2(x)
            x = self.compress_layer(x)  # Compress if modality is audio

        # Process visual input
        elif self.modality == 'visual':
            x = x.view(-1, 3, 224, 224)  # Reshape if video
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.relu(x)
            x = self.maxpool(x)
            x = self.layer1(x)
            x = self.layer2(x)
            x = self.compress_layer(x)  # Compress if modality is visual
